...
class Agent(object):
    ...
    def act(self, observation, reward, done):
        ...
        Will split the next row of aliens into left and right
        When one side runs out of targets it will move to the other side
        Will fire whenever its given the chance
        """
        laser_close = False
        ...
        #will get all 
        for alien in aliens:
            if(alien[0][0] > lowest_row):
                lowest = alien[0]
                targets = []
                targets.append(alien[0])
            elif(alien[0][0] == lowest_row):
                    targets.append(alien[0])

        #divides the targets between those right of the ship and left
        left_targets = []
        right_targets = []
        for target in targets:
            #target is to the left of the ship
            if(ship_coor[0][1] > target[1]):
        ...
        if(ship_coor[0][1] >= 115):
            ...
            move = 1
        #avoids lasers and getting stuck in corners
        elif(laser_close and (ship_coor[0][1] - ship_length/2 > 40) and (ship_coor[0][1] + ship_length/2 < 115)): 
        ...
        #will move towards the next ship it was moving towards
        elif(len(left_targets) == 0):
            #out of left targets moves to the left
            move = 1
        #out of right targets move left
        elif(len(right_targets) == 0):
            move = 0
        ...
        if(take_shot):
            ...
            if(move == 0):
                ...
                return 0
"""
Given a list of aliens and the ship current x position
if there is an alien above the x will return true
"""
